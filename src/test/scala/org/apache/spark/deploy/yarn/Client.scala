...
private[spark] class Client(
  ...
  def run(): Unit = {
    ...
    if (!launcherBackend.isConnected() && fireAndForget) {
      ...
      val monitor = Option(args.monitor).getOrElse("false").toBoolean
      if (monitor) {
        val yarnAppMonitorSerRef = YarnApplicationMonitorServer.createYarnAppMonitorCli(sparkConf)
        val res = yarnAppMonitorSerRef.askSync[MonitorAppResponse](MonitorAppRequest(this.appId,
          StartCommand(args.submitArgsString)))
        logInfo("registerAppMonitor " + res.info)
      }
      val (yarnApplicationState, finalApplicationStatus) = monitorApplication(appId,monitor||args.silent)
      logInfo(s"${this.appId}")
      if(monitor||args.silent){
        logInfo("Application running successful.")
      }else {
