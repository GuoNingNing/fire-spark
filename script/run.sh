...
function get_param(){
	...
	test ! -f "$user_proper_file" && { echo "Properties $user_proper_file file not set">&2;exit 2; }
        local v=$(grep "^$n=" $user_proper_file | head -1 | awk -F '=' '{s="";for(i=2;i<=NF;i++){if(s){s=s"="$i}else{s=$i}};print s}')
        test "x$v" == "x" && test "x$d" != "x" && v="$d"
        test "x$v" == "x" && { echo "$n not set">&2;exit 2; }
        eval "$var=\"$v\""
}
function set_jars(){
	local jar=""
	test ! -d "$lib_path" && { echo "lib_path $lib_path not found.">&2;exit 2; }
...
function check_run() {
    ...
    if test ${#appids[@]} -ne 0;then
        ...
        echo `date `" Spark app $appname already running. $appid" >&2;
        exit 2;
    fi
}
...
function set_abs_lib(){
        local p=$1
        local lp=$2
        test $(echo $p | grep -E '\.properties$' | wc -l) -eq 0 && { echo "$p the file needs to be of type properties">&2;exit 2; }
...
function set_default_lib(){
	...
	if [ $(echo $1 | grep -E "common/conf/online/$p$" | wc -l) -ne 0 ];then
		echo "../../../lib"
	elif [ $(echo $1 | grep -E "conf/online/$p$" | wc -l) -ne 0 ];then
		echo "../../lib"
...
function set_conf_dir(){
	if [ -f "conf/$1" ];then
		...
		echo "conf/online/$1"
    elif [ -f "common/conf/online/$1" ];then
		echo "common/conf/online/$1"
	else
		echo "$1"
	fi
}
...
function main(){
	...
	test ! -f "$proper" && { echo "file $proper not found">&2;exit 2; }
	...
	test ! -f "$main_jar" && { echo "$main_jar file does not exist">&2;exit 2; }
	#test "x$self_param" != "x" && self_params=($(echo $self_param | awk -F ',' '{for(i=1;i<=NF;i++){print $i}}'))

	local is_stop=""
	local last_param=${@: -1}
	test "x$last_param" == "xstop" && is_stop="stop" && shift
	check_run $is_stop
	set_jars
...
#	echo "spark-submit $main_parameter"
	...
	echo "sudo -u hdfs spark2-submit $main_parameter $@"
	spark2-submit $main_parameter "$@"
}
